-- Quote local edit tracking (PrintOS only — no Bina update)
alter table public.quotes
  add column if not exists edit_count integer not null default 0,
  add column if not exists last_edited_at timestamptz,
  add column if not exists last_edited_by text;

-- History search: expose edit fields + item dept/price mode for list badge + PDF
      ...
      (
        select coalesce(jsonb_agg(
          jsonb_build_object(
            ...
            'bina_item_code', qi.bina_item_code,
            'department', qi.department,
            'price_entry_mode', qi.price_entry_mode
